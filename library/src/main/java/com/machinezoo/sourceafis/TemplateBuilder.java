...
class TemplateBuilder {
	...
	DoubleMap readImage(final byte[] serialized) {
		android.graphics.Bitmap buffered = BitmapFactory.decodeByteArray(serialized, 0, serialized.length);
		if (buffered == null)
			throw new IllegalArgumentException("Unsupported image format");
		int width = buffered.getWidth();
		int height = buffered.getHeight();
		int[] pixels = new int[width * height];
		buffered.getPixels(pixels, 0, buffered.getWidth(), 0, 0, buffered.getWidth(), buffered
				.getHeight());
	...
	private BooleanMap filterRelativeContrast(DoubleMap contrast, BlockMap blocks) {
		...
		double averageContrast = StreamSupport.stream(sortedContrast).mapToDouble(n -> n)
	...
	private void collectMinutiae(Skeleton skeleton, MinutiaType type) {
		minutiae = RefStreams.concat(
				...
				StreamSupport.stream(skeleton.minutiae)
						...
						.map((java8.util.function.Function<SkeletonMinutia, Object>) m -> new Minutia(m.position, m.ridges.get(0).direction(), type)))
				.toArray(Minutia[]::new);
	}
